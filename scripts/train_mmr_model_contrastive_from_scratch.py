...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Jointly train LatentRouter and model-contrastive visual rereading from scratch."
    )
    ...
    parser.add_argument("--output-dir", default="artifacts/model_contrastive_from_scratch")
...
def main() -> None:
    ...
    for seed in args.seeds:
        print(f"Training joint model from scratch, seed={seed}", flush=True)
        ...
        print(
            f"seed={seed} baseline={summary['baseline_nAUC']:.6f} "
            f"joint={summary['joint_nAUC']:.6f} delta={summary['delta_vs_baseline']:+.6f}",
            flush=True,
        )

    aggregate = {
        "seed_count": len(summaries),
        "mean_baseline_nAUC": sum(row["baseline_nAUC"] for row in summaries) / len(summaries),
        "mean_joint_nAUC": sum(row["joint_nAUC"] for row in summaries) / len(summaries),
